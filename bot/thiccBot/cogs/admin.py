...
class Admin:
    ...
    async def set_bot_admin(self, ctx, role_name: str):
        ...
        if desired_role is None:
            await ctx.send(f'role with name ({role_name}) not found, make sure spelling and capitilization are the same')
            return
        async with self.bot.backend_request('put', f'/servers/{guild.id}', json={'admin_role': desired_role.id}) as r:
            if r.status == 200:
                await ctx.send(f'set the bot admin role to {desired_role.name}, any one who has this role or one above it can run any admin only command')
            else:
