...
class ThiccBot(commands.Bot):
    ...
    def backend_request(self, method: str, api_end_point: str, **kwargs) -> aiohttp.ClientResponse:
    ...
    async def add_guild(self, guild):
        async with self.backend_request('post', '/servers', data={'name': guild.name, 'id': guild.id}) as r:
            data = await r.json()
            if 'command_prefix' in data and data['command_prefix'] is not None:
                ...
                log.info('server: %s, id: %s, has command_prefix: %s', data['name'], data['id'], data['command_prefix'])
